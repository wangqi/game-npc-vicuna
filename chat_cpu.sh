...
if [[ DEBUG -eq 1 ]]
then
    jurigged -v tools/chat.py --model_path $BASE_MODEL --lora_path $LORA_PATH --force_cpu --use_local $USE_LOCAL\
    --use_typewriter 0 \
    --share_link 0 
else
python tools/chat.py --model_path $BASE_MODEL --lora_path $LORA_PATH --force_cpu --use_local $USE_LOCAL\
    --use_typewriter 0 \
    --share_link 0 
fi