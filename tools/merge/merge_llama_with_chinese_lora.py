...
if __name__=='__main__':
    ...
    for lora_index, lora_model_path in enumerate(lora_model_paths):
        ...
        try:
            ...
            previous_lora_model = lora_model_path
            previous_tokenizer = tokenizer
        except Exception:
            if previous_tokenizer is not None:
                print("Can't load tokenizer for LoRA model: {}. Using previous tokenizer {}.".
                      format(lora_model_path, previous_lora_model))
                tokenizer = previous_tokenizer
            else:
                print("Try to use base model's tokenizer. {}".format(base_model_path))
                tokenizer = LlamaTokenizer.from_pretrained(base_model_path)
        if base_model.get_input_embeddings().weight.size(0) != len(tokenizer):
            tokenizer_size = len(tokenizer)
            if tokenizer_size != model_size:
                print("WARNING: tokenizer size does not match model size. tokenizer size: {}, model size: {}".
                      format(tokenizer_size, model_size))
    ...
    output_type_list = ['huggingface','pth']
