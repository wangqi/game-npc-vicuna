...
if [ -e "models/$MODEL_NAME/$MODEL_FILE" ]; then
    ...
    # python tools/convert_ggml.py --outtype $OUT_TYPE --outfile models/$MODEL_NAME/ggml-$OUT_TYPE.bin models/$MODEL_NAME/$MODEL_FILE
else
...
# ./tools/quantize models/$MODEL_NAME/ggml-$OUT_TYPE.bin models/$MODEL_NAME/ggml-$QUANT_TYPE.bin $QUANT_TYPE

echo "Quantized the models/$MODEL_NAME/$MODEL using $QUANT_TYPE"
echo python -m gptq.llama models/$MODEL_NAME/ c4 --wbits 4 --true-sequential --groupsize 128 --save models/$MODEL_NAME/gptq-4bit-128g.pt
CUDA_VISIBLE_DEVICES=0 python -m gptq.llama models/$MODEL_NAME/ c4 --wbits 4 --true-sequential --groupsize 128 \
  --save_safetensors models/$MODEL_NAME/gptq-4bit-128g.safetensors
  # --save models/$MODEL_NAME/gptq-4bit-128g.pt
